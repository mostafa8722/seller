...
export default {
    ...
    setup(){
        ...
        const basicModels = ref({fields:[{name:"نام و نام خانوادگی",value:{value:null,valid:true}, deactive:true},{name:"نام کاربری",value:{value:null,valid:true}, deactive:true},{name:"نام کاربری",value:{value:null,valid:true}, deactive:true},{name:"شماره ثابت",value:{value:null,valid:true}, deactive:true},{name:"شماره موبایل",value:{value:null,valid:true}, deactive:true},{name:"تاریخ تولد",value:{value:null,valid:true}, deactive:true},{name:"جنسیت",value:{value:null,valid:true}, deactive:true}]})
        const secondaryModels = ref({fields:[{addressModal:true,name:"آدرس فروشگاه",value:{value:null,valid:true}, deactive:true},{name:"آپلود جواز کسب",value:{value:null,valid:true}, deactive:true},{name:"ساعات کاری",value:{value:null,valid:true}, deactive:true},{name:"محدوده ی ارسال",value:{value:null,valid:true}, deactive:true},{name:"روش ارسال",value:{value:null,valid:true}, deactive:true}]})
        const menuItems = ref([{text:'اطلاعات شخصی',icon:'user.svg',active:true},{text:'اعلان ها',icon:'black-bell.svg',active:false},{text:'فعالیت حساب',icon:'activity.svg',active:false},{text:'تنظیمات امنیتی',icon:'lock.svg',active:false},{text:'تغییر رمز عبور',icon:'shield.svg',active:false},{text:'اطلاعات حساب',icon:'dollar.svg',active:false}])
        const notifModels = ref([{label:"هر وقت با فعالیت غیر معمول روبرو شدید به من اطلاع دهید",value:{value:false,valid:true},name:'unusual-activity'},{label:"هر وقت شخص دیگری با مشخصات من وارد شد اطلاع دهید",value:{value:false,valid:true},name:'break-in'}])
        const newsModels = ref([{label:"درباره ی فروش و خبر های جدید از طریق ایمیل اطلاع دهید",value:{value:false,valid:true},name:'news'},{label:"درباره ی بروز رسانی های جدید به من اطلاع دهید",value:{value:false,valid:true},name:'update'},{label:"درباره ی نکات استفاده از حساب به من ایمیل بزنید",value:{value:false,valid:true},name:'guide'},])
        const financialModels = ref([{name:"بانک",value:{value:null,valid:true}, deactive:true},{name:"شماره حساب",value:{value:null,valid:true}, deactive:true},{name:"شماره شبا",value:{value:null,valid:true}, deactive:true},{name:"شماره کارت",value:{value:null,valid:true}, deactive:true}])
        const aBankAccount = ref({bank:{value:null,valid:true},accountNo:{value:null,valid:true},shebaNo:{value:null,valid:true},cardNo:{value:null,valid:true}})
        const workTime = ref({edit:false,deactive:true,fields:[{name:'از ساعت',value:{value:null,valid:true}},{name:'تا ساعت',value:{value:null,valid:true}}]})
        const theWorkDay = ref({value:null,valid:true})
        const preparationTime = ref({fields:[{name:"زمان آماده سازی",value:{value:null,valid:true}}],edit:false, deactive:true})
        ...
        const getSellerInfos = () => {
            ...
            authService.value.receive('seller/base/distance',{},(s,d)=>{
                if(s == 200){
                    if(d.data != [] && d.data != null){
                        sendCost.fields[0].value = {valid:true,value:{text: ' از ' + d.data[0].from + ' تا ' + d.data[0].to,value:d.data[0].id,id:d.data[0].id,distance_id:d.data[0].id}}
                        ...
                        if(d.data != []){
                            serviceRanges.value.map((ser)=>{
                                if(d.data.service_range_id == ser.id){
                                    serviceRange.value = {deactive:true,edit:false,fields:[{value:{value:{value:ser.id,text:ser.text},valid:true}}]}
                                }
                            })
                        }
                    }
