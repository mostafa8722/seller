  ...
  <div class="seller-page">
    ...
    <div class="seller-form mt-4">
      <div class="seller-tab" v-if="authStep == 1">
        <v-row>
          <v-col>
            <custom-input kind="text" key="1" container="full-width ml-3 mt-3" classes="full-width"
                          v-bind:theModel.sync="mobile" label="شماره موبایل"></custom-input>

          </v-col>
        </v-row>
        <v-row>
          <v-col>
            <custom-input kind="text" key="2" container="full-width ml-3 mt-3" classes="full-width"
                          v-bind:theModel.sync="password" label="گذرواژه"></custom-input>
      ...
      <div class="seller-tab" v-if="(authStep == 2 || authStep == 9)">
        <custom-input kind="text" key="3" container="full-width ml-3 mt-3" classes="full-width"
                      v-bind:theModel.sync="mobile" label="شماره موبایل"></custom-input>
      </div>
      <div class="seller-tab" v-if="(authStep == 3 || authStep == 10)">
        <custom-input kind="text" key="4" container="full-width ml-3 mt-3" classes="full-width"
                      v-bind:theModel.sync="code" label="کد"></custom-input>
        <p class="tool-paragrapgh" v-if="authStep == 3" @click="resendCode">ارسال دوباره</p>
      </div>
      <div class="seller-tab" v-if="(authStep == 4 || authStep == 11)">
        <custom-input kind="text" v-if="authStep == 4" key="5" container="full-width ml-3 mt-3" classes="full-width"
                      v-bind:theModel.sync="name" label="نام فروشگاه"></custom-input>
        <custom-input kind="text" key="6" container="full-width ml-3 mt-3" classes="full-width"
                      v-bind:theModel.sync="password" label="گذرواژه"></custom-input>
        <custom-input kind="text" key="7" container="full-width ml-3 mt-3" classes="full-width"
                      v-bind:theModel.sync="rePassword" label="گذر واژه را مجددا وارد کنید"></custom-input>
      </div>
